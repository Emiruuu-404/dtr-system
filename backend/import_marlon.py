...
def import_marlon():
    print("🎯 Importing specifically for Caisip, Marlon S. ...")
    intern = Intern.objects.get(student_id="22-0006")
    ...
    # For Marlon, his block is roughly rows 212 to 238
    processed_count = 0
    for idx in range(211, 239): 
        row = rows[idx]
        ...
        for mg in GROUPS:
            d_idx = mg["d"] + offset
            if len(row) > d_idx and row[d_idx].strip().replace('.','').isdigit():
                try:
                    day_str = row[d_idx].strip()
                    if '.' in day_str: day = int(float(day_str))
                    else: day = int(day_str)
                except: continue
    ...
    print(f"\n✨ Done! Marlon Total Hours: {intern.total_hours}")
